...
### Gantt
![간트](https://user-images.githubusercontent.com/64582209/184828162-e3b6eafe-54f7-4d66-b251-037d969e783c.JPG)
...
![전자결재대시보드](https://user-images.githubusercontent.com/64582209/185057129-4e3e1b57-5745-4d57-8545-8864ae8e5fe2.JPG)



## 프로젝트중 발생한 이슈사항 / 느낀점
* 업무목록 구현시에 성능을 고려하지 않고 쿼리문 작성을 하여 Service단에서 복잡한 로직을 통해 구현하려다 보니 적은 데이터를 출력하는데도 성능이 많이 떨어지는 현상이 발생했습니다. 
> 복잡하더라도 잘 짜놓은 쿼리문의 성능이 훨씬 좋다는 것을 깨닫고 join과 서브쿼리를 적용해 쿼리문을 수정해 문제를 해결했다. SQL공부를 더 해야겠음을 느꼈습니다.
* 거의 모든 페이지에서 Handlabars 템플릿을 이용해 비동기처리를 하다보니 코드는 같지만 발생하는 event가 다른 요소에 대해 중복코드를 간과하고 함수를 작성했습니다.
> pagination 템플릿을 공통으로 사용할 수 있게 만들고 ajax콜백함수에서 객체를 조작해 공통 템플릿을 사용할 수 있는 pagination객체를 만들어서 문제를 해결했다. 비동기로 처리해야 할 때 항상 중복코드를 고려해 코드를 작성해야겠음을 느꼈습니다.
* 시간이 촉박해 프로젝트 설계단계에서 나중에 생길 문제에 대해 깊게 생각하지않고 빠르게 개발단계로 넘어가 개발하면서 요구사항을 수정하는 일이 많았습니다.
> 기술과 문제해결능력도 중요하지만 내가 개발하려하는 기능의 요구사항을 정확히 인지하고 개발단계로 넘어가는것이 제일 중요함을 느꼈습니다.
